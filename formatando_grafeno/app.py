    ...
    for encoding in encodings:
        for sep in separadores:
            try:
                ...
                continue
    print("Não foi possível ler o arquivo CSV com os encodings e separadores testados")
...
def processar_dataframe(df, arquivo, nome_planilha):
    ...
    if linha_cabecalho is not None:
        ...
        # Renomear colunas com base na normalização
        ...
        if not all([col_data, col_valor, col_saldo]):
            print("As colunas esperadas não foram encontradas após normalização.")
            return
        ...
        print("\nDados extraídos e formatados (mostrando primeiras linhas):")
        print(df_final.head())

        salvar = input("\nDeseja salvar os dados extraídos em um novo arquivo? (s/n): ").lower()
        if salvar == 's':
            ...
            print(f"Novo arquivo criado com os dados extraídos: {nome_saida}")
    else:
        print("\nCabeçalhos não encontrados nesta planilha/arquivo.")
        print("As primeiras linhas do arquivo são:")
        print(df.head())

def main():
    print("Selecione o arquivo para análise (Excel ou CSV)")
    print("AVISO: O arquivo original NÃO será modificado.")
